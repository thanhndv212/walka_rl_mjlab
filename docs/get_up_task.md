...
| **v1.7** *(current)* | **Reverted v1.5/v1.6 back to v1.4**, added `upper_body_upright` (weight 3.0) — rewards the thorax's own verticality directly, gated on `stand_on_feet`'s condition | Addresses the v1.4 finding at its source instead of through the hand-contact proxy v1.5/v1.6 used. CPU-verified: fires (~0.93–0.997) once both feet are planted and the pelvis is at standing height, exactly zero before that gate opens. **GPU burst done** (commit `55907f4`, W&B `td3tfmgr`, `num_envs=1024`, 1500 iterations, ~$0.15 on a rented RTX 4090, 36.9M total env-steps): operationally clean — no crashes/NaNs, `upper_body_upright` climbed steadily to 1.45 by the final iteration in step with `stand_on_feet` (1.23) and `task_progress` (3.66), no destabilizing divergence like v1.5's exploit pattern; training-log `standing_success` reached 0.49. But `scripts/eval_fallen_recovery.py` against this checkpoint shows only **1.7% genuine recovery (1/60)**, +0.010m over baseline — statistically indistinguishable from doing nothing, same as v1.6. **This is inconclusive, not a bad sign**: at 36.9M env-steps this burst is ~7× smaller than the point where v1.6's *own* training-log metric first peaked (~260M steps), and roughly 1.5% of v1.4's full 10k/4096-env campaign (which itself only crossed from curriculum-assisted "success" to genuine recovery late in training — its own metric dipped to ~0.03 at 75% through before finishing at 0.82/100%). A burst this short was never going to show real recovery skill on the fallen-pose eval; it validates operational correctness (the actual purpose of the CPU/burst tiers per "Implementation and validation roadmap" below), not the posture fix itself. **Next step, not yet done**: a full-scale campaign (v1.4-comparable: 4096+ envs, 10-15k iterations) is required before `eval_fallen_recovery.py` can say anything meaningful about whether `upper_body_upright` actually fixes the bent-waist posture. |
