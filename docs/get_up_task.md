...
**Status (post-v1.7):** v1.4 (W&B `c3b1ytc6`, `model_9999.pt`) is still
the best checkpoint this task has produced — **100% genuine recovery**
from genuinely fallen poses (`scripts/eval_fallen_recovery.py`) — but its
standing pose is wrong: a bent-forward waist, propped by a hand left on
the ground. **Three consecutive attempts to fix that posture have now
failed at full scale**: v1.5 (hand-contact penalty) collapsed to ~0%
...
shape. v1.5/v1.6 are reverted in code; **v1.7's `upper_body_upright` term
is still present in the current reward set** pending a decision on whether
to revert it too or investigate/tune it further (weight, gating condition)
given it hasn't been root-caused yet. The bent-waist posture problem
remains open regardless. See "Version history and training campaign log"
